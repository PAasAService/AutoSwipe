...
const prefsSchema = z.object({
  ...
  preferredBrands: z.array(z.string().max(60)).max(20).default([]),
  preferredModels: z.array(z.string().max(60)).max(30).default([]),
  fuelPreferences: z.array(z.enum(['GASOLINE', 'DIESEL', 'HYBRID', 'ELECTRIC', 'PLUG_IN_HYBRID'])).max(5).default([]),
  transmissionPreferences: z.array(z.enum(['AUTOMATIC', 'MANUAL'])).max(2).default([]),
  vehicleTypes: z.array(z.enum(['SEDAN', 'SUV', 'HATCHBACK', 'COUPE', 'CONVERTIBLE', 'MINIVAN', 'PICKUP', 'WAGON', 'CROSSOVER'])).max(9).default([]),
...
export async function PUT(req: NextRequest) {
  ...
  const dbData = {
    ...
    transmissionPreferences: JSON.stringify(data.transmissionPreferences),
