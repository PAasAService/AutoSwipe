...
export type ListingEditorProps =
  | { mode: 'create' }
  | { mode: 'edit'; listingId: string; initialListing: ListingEditorInitial }
...
function buildFormFromListing(L: ListingEditorInitial): FormData {
  const imgs = [...L.images]
  ...
  return {
    ...
    price: L.price,
    location: L.location,
    fuelType: L.fuelType as FuelType | '',
    fuelConsumption: L.fuelConsumption,
    vehicleType: L.vehicleType as VehicleType | '',
    transmission: (L.transmission || 'AUTOMATIC') as Transmission,
    engineSize: L.engineSize ?? 0,
    color: L.color ?? '',
    doors: L.doors ?? 4,
    seats: L.seats ?? 5,
    vehicleCategory: 'car',
...
export function ListingEditor(props: ListingEditorProps) {
  ...
  const [form, setForm] = useState<FormData>(() =>
    props.mode === 'edit' ? buildFormFromListing(props.initialListing) : INITIAL,
  )
  ...
  const [plateNumber,       setPlateNumber]       = useState(() =>
    props.mode === 'edit' ? props.initialListing.plateNumber ?? '' : '',
  )
  const [isGovVerified,     setIsGovVerified]     = useState(() =>
    props.mode === 'edit' ? props.initialListing.isGovVerified : false,
  )
  ...
  const lastFetchRef = useRef(
    props.mode === 'edit'
      ? `${props.initialListing.brand}__${props.initialListing.model}__${props.initialListing.year}`
      : '',
  )
  const skipSpecsInitRef = useRef(props.mode === 'edit')
  ...
  useEffect(() => {
    ...
    if (skipSpecsInitRef.current) {
      skipSpecsInitRef.current = false
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'שגיאה בשמירת המודעה')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-surface-container-lowest px-5 pb-10 pt-4" dir="rtl">
      ...
      <div className={step === 4 ? 'mt-2' : 'hidden'}>
        <ImageUploader
          seedKey={props.mode === 'edit' ? props.listingId : undefined}
          initialImages={
            props.mode === 'edit'
              ? [...props.initialListing.images]
                  .sort((a, b) => a.order - b.order)
                  .map((im) => ({ path: im.path }))
              : undefined
          }
          ...
          >
            {loading
              ...
              : uploadsBusy
                ...
                : <><Check className="w-5 h-5" /> {props.mode === 'edit' ? 'שמור שינויים' : 'פרסם מודעה'}</>
            }
          </button>
        )}
      </div>
